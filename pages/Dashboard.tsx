...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  if (isGuest && links.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-20 text-center">
         <h2 className="text-xl font-bold text-gray-900 dark:text-white">{t('dash.guest.empty')}</h2>
         <p className="mt-2 text-gray-600 dark:text-gray-400">{t('dash.guest.empty_desc')}</p>
         <Link to="/" className="mt-4 inline-block text-indigo-600 dark:text-indigo-400 font-medium">{t('dash.guest.create_new')} &rarr;</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {showBulkInfo && (
        <BulkInfoModal onClose={() => setShowBulkInfo(false)} />
      )}

      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
          {isGuest ? t('dash.title.guest') : t('dash.title.user')}
        </h1>
        {isGuest && (
        ...
        {!isGuest && (
          <div className="flex items-center gap-2 mt-4 md:mt-0">
            <button
              ...
              className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none transition-colors"
            >
            ...
            <button
              ...
              className="inline-flex items-center justify-center h-9 w-9 border border-gray-300 dark:border-gray-600 rounded-full text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors focus:outline-none"
              title="Hướng dẫn"
            >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
            <div>
              <h2 className="text-lg font-medium text-gray-900 dark:text-white flex items-center gap-2">
                {t('dash.chart.title')}: <span className="text-indigo-600 dark:text-indigo-400">/{selectedLink.slug}</span>
                {isExpired(selectedLink.expiresAt) && <span className="px-2 py-0.5 rounded text-xs bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200">{t('dash.chart.expired')}</span>}
              </h2>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-4 sm:mt-0">
               <div className="flex items-center gap-1">
                 <label className="text-xs text-gray-500 dark:text-gray-400">{t('dash.filter.from')}</label>
                 <input 
                   type="date" 
               ...
               <div className="flex items-center gap-1">
                 <label className="text-xs text-gray-500 dark:text-gray-400">{t('dash.filter.to')}</label>
                 <input 
                   type="date" 
                   className="text-sm border border-gray-300 dark:border-gray-600 rounded px-2 py-1 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
      ...
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 transition-colors">
        <div className="px-4 py-5 border-b border-gray-200 dark:border-gray-700 sm:px-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white">{t('dash.table.title')}</h3>
          {!isGuest && (
            <input 
              type="text" 
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm" onClick={e => e.stopPropagation()}>
                    {editingExpiry === link.id ? (
                      <div className="flex items-center gap-1">
                        ...
                        />
                        <button onClick={() => handleUpdateExpiry(link.id)} className="text-green-600 dark:text-green-400 text-xs font-bold">{t('common.save')}</button>
                        <button onClick={() => setEditingExpiry(null)} className="text-gray-500 dark:text-gray-400 text-xs">{t('common.cancel')}</button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-2">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${expired ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'}`}>
                          {expired ? t('dash.status.expired') : t('dash.status.active')}
                        </span>
                        <div className="text-xs text-gray-500 dark:text-gray-400">
                           {link.expiresAt ? new Date(link.expiresAt).toLocaleString() : t('dash.status.forever')}
                        </div>
                        <button 
                          onClick={() => {
